...
void* reader_thread(void* arg)
{
	...
	while ((a = read_input()) != EOF) {
		printf("sss");
		while (inputB.writeI == inputB.length) {

			printf("sss");
			pthread_cond_wait(&writeEmpty, &mutexBuffer);
		}
	//	printf("One ")
...
void* IC_thread(void* arg)
{
	...
	//	printf("%d ", inputB.readI);
		//sem_wait(&empty);
		pthread_mutex_lock(&mutexBuffer);
		while (count < 500) {
			pthread_cond_wait(&readEmpty, &mutexBuffer);
			printf("%d ", inputB.readI);
		}
		char result;
		count_input(readIB());
	...
	//	inputB.readI++;
		printf(" REEE ");
		pthread_mutex_unlock(&mutexBuffer);
		pthread_cond_signal(&writeEmpty);
		//inputB.readI++;
